class Course < ApplicationRecord
    has_many :participants, dependent: :destroy # Ja dzēš kursu, izdzēsīs arī participantus, kuri saistīti ar konkrēto kursu
    ...
    def start_and_end_dates
      # On creating course checks if start date isnt today 
        if begins_at.present? && begins_at >= (begins_at + 1.day) #Present asks if theres some value for begins_at
            errors.add(:begins_at, "cant be today or any date from the past")
        end  

      # On creating course checks if end date isnt today   
        if ends_at.present? && ends_at >= (ends_at + 1.day)
